...
Encountered a problem : grammars for the metalanguage and the grammar language are incompatible (# means either comment or symbol).

Solution :

Write a parser for grammars in the metalanguage directly, so it is extendable and can allow namespaces.
This parser outputs a tree representation for a grammar, that should then be able to be evaluated to parse any input.
It however means that this parser should also know how to parse the metalanguage itself, to represent grammar actions.

Next steps :
- Write the parser
- Feed it a namespaced grammar that contains both the grammar grammar and the metalanguage grammar to create a tree.
- Evaluate this tree by feeding it that namespaced grammar and it should return a copy of itself.
- Evaluating a metaprogram through the namespaced tree and through the peg parser (generated by minproto.leg) should produce the same tree representation for that metaprogram.
